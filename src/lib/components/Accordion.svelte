<script lang="ts">
  ...
  let { question, answer, linkText, link }: Props = $props();
...
<div class="accordion">
    ...
    <div>
      <svg
        ...
        style="transform: rotate({isOpen
          ? 180
          : 0}deg); transition: transform 300ms ease-out;"
      >
        <path
  ...
  >
    <div class="answer text-base" bind:clientHeight={innerHeight}>
      {#if linkText && link}
        {@html "<span>"}{answer}<a href={link} class="underline">{linkText}</a
        >{@html "</span>"}
      {:else}
        {@html "<span>"}{answer}{@html "</span>"}
      {/if}
    </div>
  </div>
</div>
