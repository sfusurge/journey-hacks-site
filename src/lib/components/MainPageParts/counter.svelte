<script>
  ...
  let difference = $derived(
    new Date(Math.max(0, hackathonDate.getTime() - nowDate.getTime()))
  );
  ...
  let days = $derived(
    new String(Math.floor(difference.getTime() / msInDay)).padStart(2, "0")
  );
  let hours = $derived(
    new String(
      Math.floor((difference.getTime() % msInDay) / msInHours)
    ).padStart(2, "0")
  );
  let minutes = $derived(
    new String(
      Math.floor((difference.getTime() % msInHours) / msInMinute)
    ).padStart(2, "0")
  );
...
>
  <img
    alt="ceiling"
    src="/counter/ceiling.png"
    class="relative w-full h-auto hidden md:block"
  />

  <img
    alt="ceiling"
    src="/counter/ceiling-mobile.png"
    class="relative w-full h-auto block md:hidden"
  />

  <div class="absolute top-0 left-0 w-full h-full">
    <img
      alt="foreground"
      src="/counter/counter.png"
  ...
  <div class="absolute top-0 left-0 w-full h-full">
    ...
    >
      <div
        class="font-coiny text-[12vw] md:text-[6vw] text-center w-[2ch] block mx-auto"
        id="seconds-digit-1"
      >
        {days}
      </div>
